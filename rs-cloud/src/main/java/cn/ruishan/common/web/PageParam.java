...
/**
 * @desc: 列表分页、排序、搜索通用接收参数封装
 * @author: longgang.lei
 * @time: 2021-04-05 13:24
 */
public class PageParam<T> extends PageDTO<T> {
    ...
    public PageParam<T> init(HttpServletRequest request) {
        ...
        while (names.hasMoreElements()) {
            ...
            if (FILED_PAGE.equals(name)) {
                ...
                sortValue = (needToLine ? StrUtil.toUnderlineCase(value) : value);
     ...
     * create_date asc
     */
    public PageParam<T> setDefaultOrderByCreateTime() {
        if (ArrayUtil.isEmpty(getOrderDescs()) && ArrayUtil.isEmpty(getOrderAscs())) {
            addOrderAsc("create_time");
        }
        return this;
    }
     ...
     * 删除pageData里面指定参数
     */
    public PageParam<T> remove(String key) {
        if (pageData == null) pageData = new HashMap<>();
        pageData.remove(key);
        return this;
    }
    ...
    private void buildWrapper(QueryWrapper<T> queryWrapper, String key, String value) {
        ...
        if (Arrays.asList("id", "sortNumber", "state").contains(key) || key.endsWith("Id")) {
            queryWrapper.eq(needToLine ? StrUtil.toUnderlineCase(key) : key, value);
            ...
            queryWrapper.like(needToLine ? StrUtil.toUnderlineCase(key) : key, value);
        }
    }
    ...
    public QueryWrapper<T> getOrderWrapper(QueryWrapper<T> queryWrapper) {
        ...
        queryWrapper.orderByAsc(getOrderAscs().get(0));
        queryWrapper.orderByDesc(getOrderDescs().get(0));
        return queryWrapper;
    }
        ...
        if (orderItems != null) {
            ...
            for (OrderItem order : orderItems) {
                if (order.getColumn() != null) {
                    Function keyExtractor = t -> getFieldValue(t, needToLine ? StrUtil.toCamelCase(order.getColumn()) : order.getColumn());
                    if (comparator == null) {
                        if (order.isAsc()) {
