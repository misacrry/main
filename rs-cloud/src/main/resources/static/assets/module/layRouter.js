...
layui.define(function (exports) {
    let router = {
        ...
        init: function (options) {
            router.index = router.routerInfo(options.index).href.substring(1);
        ...
        reg: function (hash, handler) {
            if (hash) {
                ...
                if (hash instanceof Array) {
                    ...
                    hash = router.routerInfo(hash).href.substring(1);
        ...
        routerInfo: function (url) {
            ...
            let hash = url.replace(/^#+/g, '').replace(/\/+/g, '/');
            if (hash.indexOf('/') !== 0) {
                hash = '/' + hash;
            }
    ...
    function onhashchange(url, refresh) {
        let routerInfo = router.routerInfo(url);
        router.lash = routerInfo.href;
        let hash = routerInfo.href.substring(1);
        if (!hash || hash === '/') {
            hash = router.index;
