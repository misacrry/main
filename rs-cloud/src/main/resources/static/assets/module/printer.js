...
layui.define(['jquery'], function (exports) {
    let $ = layui.jquery;
    let hideClass = 'hide-print';  // 打印时隐藏
    let printingClass = 'printing';  // 正在打印
    let ieWebBrowser = '<object id="WebBrowser" classid="clsid:8856F961-340A-11D0-A96B-00C04FD705A2" width="0" height="0"></object>';
    let printer = {
        // 判断是否是ie
        isIE: function () {
    ...
    printer.print = function (param) {
        ...
        param || (param = {});
        let hide = param.hide;  // 需要隐藏的元素
        let horizontal = param.horizontal;  // 纸张是否是横向
        let iePreview = param.iePreview;  // 兼容ie打印预览
        let blank = param.blank;  // 是否打开新窗口
        let close = param.close;  // 如果是打开新窗口，打印完是否关闭
        // 设置参数默认值
        ...
        if (horizontal !== undefined) {
            let printSet = '<style type="text/css" media="print" id="page-print-set">';
        ...
        if (blank) {
            ...
            pDocument.open();
            let blankHtml = '<!DOCTYPE html>' + document.getElementsByTagName('html')[0].innerHTML;
    ...
    printer.printHtml = function (param) {
        param || (param = {});
        let html = param.html;  // 打印的html内容
        let blank = param.blank;  // 是否打开新窗口
        let close = param.close;  // 打印完是否关闭打印窗口
        let print = param.print;  // 是否自动调用打印
        let horizontal = param.horizontal;  // 纸张是否是横向
        let iePreview = param.iePreview;  // 兼容ie打印预览
        // 设置参数默认值
    ...
    printer.printPage = function (param) {
        param || (param = {});
        let htmls = param.htmls;  // 打印的内容
        let horizontal = param.horizontal;  // 纸张是否是横向
        let style = param.style;  // 打印的样式
        let padding = param.padding;  // 页边距
        let blank = param.blank;  // 是否打开新窗口
        let close = param.close;  // 打印完是否关闭打印窗口
        let print = param.print;  // 是否自动调用打印
        let width = param.width;  // 页面宽度
        let height = param.height;  // 页面高度
        let iePreview = param.iePreview;  // 兼容ie打印预览
        let isDebug = param.debug;  // 调试模式
        // 设置参数默认值
        (print === undefined) && (print = true);
        (iePreview === undefined) && (iePreview = true);
        (blank === undefined && printer.isIE()) && (blank = true);
        ...
        pWindow.focus();  // 让打印窗口获取焦点
        // 拼接打印内容
        let htmlStr = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>打印窗口</title>';
        ...
        // 拼接分页内容
        if (htmls) {
            let pageClass = isDebug ? ' page-debug' : '';  // 调试模式
    ...
    printer.getCommonCss = function (isPrinting) {
        let cssStr = ('.' + hideClass + '.' + printingClass + ' {');
    ...
    printer.makeHtml = function (param) {
        ...
        let htmlStr = '<!DOCTYPE html><html lang="en">';
