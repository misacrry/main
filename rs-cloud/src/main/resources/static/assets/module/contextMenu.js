...
layui.define(["jquery"], function (exports) {
    ...
    let contextMenu = {
        ...
        show: function (items, x, y, e) {
            let xy = 'left: ' + x + 'px; top: ' + y + 'px;';
            let htmlStr = '<div class="ctxMenu" style="' + xy + '">';
            ...
            $('.ctxMenu-item').on('mouseenter', function (e) {
                ...
                let $sub = $(this).find('>.ctxMenu-sub');
                let top = $item.offset().top - $('body,html').scrollTop();
                let left = $item.offset().left + $item.outerWidth() - $('body,html').scrollLeft();
            ...
            function getItemById(id, list) {
                for (let i = 0; i < list.length; i++) {
                    ...
                    if (id == one.itemId) {
                        ...
                        let temp = getItemById(id, one.subs);
                        if (temp) {
                            return temp;
                        }
                    }
        ...
        getCommonCss: function () {
            let cssStr = '.ctxMenu, .ctxMenu-sub {';
