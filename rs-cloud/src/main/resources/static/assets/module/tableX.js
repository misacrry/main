...
layui.define(['layer', 'table', 'laytpl', 'contextMenu'], function (exports) {
    ...
    let tbSearchAttr = 'tb-search';  // 前端搜索属性
    let tbRefreshAttr = 'tb-refresh';  // 刷新按钮属性
    let tbExportAttr = 'tb-export';  // 导出按钮属性
    let txFieldPre = 'txField_';  // templte列filed前缀
    let tableX = {};
    ...
    tableX.merges = function (tableId, indexs, fields, sort) {
        ...
        let $tb = $('[lay-filter="' + tableId + '"]+.layui-table-view>.layui-table-box>.layui-table-body>table');
        let $trs = $tb.find('>tbody>tr');
        // 循环合并每一列
    ...
    tableX.bindCtxMenu = function (tableId, items) {
        let data = table.cache[tableId];
        let elem = '#' + tableId + '+.layui-table-view .layui-table-body tr';
    ...
    tableX.exportData = function (param) {
        let cols = param.cols;  // 表头
        let data = param.data;  // 数据
        let fileName = param.fileName;  // 文件名
        let fileType = param.expType;  // 类型，xls、csv
        let option = param.option;  // url方式加载的配置
        option || (option = {});
        ...
        // 获取templet后的数据
        let fullData = tableX.parseTbData(cols, tableX.deepClone(data), true);
        for (let n = 0; n < fullData.length; n++) {
            let rowItem = [];
            for (let m = 0; m < fields.length; m++) {
                let text = fullData[n][fields[m]];
        ...
        let type = ({csv: 'text/csv', xls: 'application/vnd.ms-excel'})[fileType || 'xls'];
        let content = encodeURIComponent(titles.join(',') + '\r\n' + expData.join('\r\n'));
    ...
    tableX.exportDataX = function (param) {
        layui.use('excel', function () {
            ...
            let data = param.data;  // 数据
            let fileName = param.fileName;  // 文件名
            let fileType = param.expType;  // 类型，xls、xlsx
            let option = param.option;  // url方式加载的配置
            option || (option = {});
            ...
            table.eachCols(undefined, function (i, item) {
                if (item.type === 'normal' && !item.hide) {
                    let field = item.field || (txFieldPre + i);
                    fields.push(field);
            ...
            // 格式化数据
            let fullData = tableX.parseTbData(cols, tableX.deepClone(data), true);
            let expData = excel.filterExportData(fullData, fields);
    ...
    tableX.exportDataBack = function (url, where, method) {
        ...
        if (!method || method.toString().toLowerCase() === 'get') {
            ...
            let htmlStr = '<html><body><form id="eFrom" action="' + url + '" method="' + method + '">';
            for (let f2 in where) {
            ...
            let eFrame = document.getElementById('exportFrame');
            let eWindow = eFrame.contentWindow;
            let eDocument = eWindow.document;
            eWindow.focus();
            eDocument.open();
    ...
    tableX.render = function (param) {
        let tableId = $(param.elem).attr('lay-filter');
        param.autoSort = false;  // 关闭默认排序
        let insTb = table.render(param);  // 渲染表格
        // 排序监听
        table.on('sort(' + tableId + ')', function (obj) {
            let sortField = obj.field, sortType = obj.type; // 排序字段、类型
            let sortWhere = $.extend(param.where, {sort: sortField, order: sortType});
    ...
    tableX.renderFront = function (param) {
        let insTb, tableId = $(param.elem).attr('lay-filter');
        param.autoSort = false;  // 关闭默认排序
        // 没有field的templet列补上，因为排序必须有filed字段，否则点击排序会报错
        for (let i = 0; i < param.cols.length; i++) {
            for (let j = 0; j < param.cols[i].length; j++) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res[response.statusName] != response.statusCode) {
                    let msgText = res[response.msgName] || ('返回的数据不符合规范，正确的成功状态码 (' + response.statusName + ') 应为：' + response.statusCode);
    ...
    tableX.parseTbData = function (cols, dataList, overwrite) {
        ...
        table.eachCols(undefined, function (i, item) {
            if (item.templet) {
                let one = {field: ((item.field && (overwrite || item.field.indexOf(txFieldPre) === 0)) ? item.field : ('txField_' + i))};
        ...
        for (let i = 0; i < dataList.length; i++) {
            ...
            for (let j = 0; j < templets.length; j++) {
                let htmlStr = '<div>' + templets[j].templet(current) + '</div>';
    ...
    tableX.getTbData = function (tableId) {
        let dataList = window.tbX.cache[tableId];
        return tableX.deepClone(dataList || table.cache[tableId]);
    };

    /* 获取表格缓存的数据(不带LAY_TABLE_INDEX) */
    tableX.getTbDataEx = function (tableId) {
        let dataList = window.tbX.cache[tableId];
        dataList = tableX.deepClone(dataList || table.cache[tableId]);
        if(!dataList) dataList = [];
        for (let i = 0; i < dataList.length; i++) {
            let obj = dataList[i];
            if(obj.hasOwnProperty('LAY_TABLE_INDEX')) {
    ...
    /* 渲染刷新组件 */
    tableX.renderRefresh = function (insTb) {
        renderRefresh(insTb);  // 刷新
    }
    /* 渲染排序组件 */
    tableX.renderFrontSort = function (insTb) {
        renderFrontSort(insTb);  // 排序
    }
    /* 渲染搜索组件 */
    tableX.renderFrontSearch = function (insTb) {
        renderFrontSearch(insTb);  // 搜索
    }
    /* 渲染导出组件 */
    tableX.renderExport = function (insTb) {
        renderExport(insTb);  // 导出
    }
    ...
    // 前端搜索
    let renderFrontSearch = function (insTb) {
        let tableId = insTb.config.id, $input = $('input[' + tbSearchAttr + '="' + tableId + '"]');
        ...
        $input.off('keydown').on('keydown', function (event) {
            ...
            let searchName = $input.attr('name');  // 搜索的字段名，用逗号分隔
            let searchValue = $input.val().replace(/(^\s*)|(\s*$)/g, '');  // 搜索关键字
            let loadIndex = layer.msg('搜索中..', {icon: 16, shade: 0.01, time: 0});
            let dataList = tableX.getTbData(tableId);
            let newDataList = tableX.filterData(dataList, searchName, searchValue);
    ...
    // 前端排序
    let renderFrontSort = function (insTb) {
        let tableId = insTb.config.id;
        table.on('sort(' + tableId + ')', function (obj) {
            let sortField = obj.field, sortType = obj.type; // 排序字段、类型
            let loadIndex = layer.msg('加载中..', {icon: 16, shade: 0.01, time: 0});
            let dataList = window.tbX.cacheSearch[tableId];  // 表格搜索后的数据
            dataList || (dataList = tableX.getTbData(tableId));
            if (sortType) {
                dataList = dataList.sort(function (o1, o2) {
                    let o1Str = o1[sortField], o2Str = o2[sortField];
                    if (sortType === 'asc') {  // 升序
    ...
    // 表格刷新按鈕
    let renderRefresh = function (insTb) {
    ...
    // 渲染导出按钮
    let renderExport = function (insTb) {
        let tableId = insTb.config.id;
        $('[' + tbExportAttr + '="' + tableId + '"]').off('click').on('click', function (event) {
            ...
            let htmlStr = '<div class="tbx-dropdown-menu">';
            ...
            $('.tbx-dropdown-menu').off('click').on('click', '.tbx-dropdown-menu-item', function (event) {
                ...
                if (type === 'check') {
                    let checkRows = table.checkStatus(tableId);
    ...
    /* css样式 */
    let getCommonCss = function () {
        let cssStr = '.tbx-dropdown-btn {';
