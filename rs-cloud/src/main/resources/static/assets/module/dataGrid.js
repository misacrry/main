...
layui.define(['laytpl', 'laypage', 'form'], function (exports) {
    ...
    let form = layui.form;
    let MOD_NAME = 'DataGrid';
    let loadingClass = 'ew-datagrid-loading';
    let itemClass = 'ew-datagrid-item';
    let loadMoreLoadingClass = 'ew-loading', loadMoreEndClass = 'ew-more-end';
    // 默认分页参数
    let defaultPage = {
    ...
    DataGrid.prototype.init = function () {
        ...
        if (options.checkAllElem) {
            let $old = components.$checkAll.find('input[lay-filter="' + components.checkAllFilter + '"]');
        ...
        if (options.url) {  // url加载模式
            ...
            if (options.loadMore) {  // 开启加载更多
                ...
                options.page.jump = function (obj, first) {
                    options.showLoading();
                    let start = (obj.curr - 1) * options.page.limit;
                    let end = start + options.page.limit;
                    if (end > options.data.length) end = options.data.length;
                    let currData = [];
                    for (let i = start; i < end; i++) currData.push(options.data[i]);
    ...
    DataGrid.prototype.bindEvents = function () {
        ...
        /* 事件公共返回对象 */
        let commonMember = function (ext) {
            let $item = $(this);
            if (!$item.hasClass(itemClass)) {
                let $temp = $item.parent('.' + itemClass);
        ...
        form.on('checkbox(' + components.checkAllFilter + ')', function (data) {
            ...
            components.$elem.find('input[name="' + components.checkboxFilter + '"]').each(function () {
                ...
                let $temp = $this.next('.layui-form-checkbox');
                if (checked) $temp.addClass('layui-form-checked');
                else $temp.removeClass('layui-form-checked');
            });
            // 同步数据
            for (let i = 0; i < that.options.data.length; i++) that.options.data[i].LAY_CHECKED = checked;
    ...
    DataGrid.prototype.changeLoadMore = function (state) {
        let options = this.options;
        let components = this.getComponents();
        let $loadMoreText = components.$loadMore.find('.ew-loadmore-text');
    ...
    DataGrid.prototype.update = function (index, fields, type) {
        let that = this;
        let components = this.getComponents();
        let $item = components.$elem.children('[data-index="' + index + '"]');
    ...
    DataGrid.prototype.checkStatus = function () {
        let that = this;
        let components = this.getComponents();
        let checkboxFilter = components.checkboxFilter;
        let radioFilter = components.radioFilter;
        let list = [];
        // 获取单选框选中数据
        let $radio = components.$elem.find('input[name="' + radioFilter + '"]');
        if ($radio.length > 0) {
            let index = $radio.filter(':checked').val();
            if (index !== undefined) {
                let d = that.getData(index);
                if (d) list.push(d);
            }
        } else {  // 获取复选框数据
    ...
    DataGrid.prototype.checkChooseAllCB = function () {
        let components = this.getComponents();
        let $cbAll = components.$checkAll.find('input[lay-filter="' + components.checkAllFilter + '"]');
        let isCheckAll = this.options.data.length !== 0;
    ...
    dg.autoRender = function (elem) {
        $(elem || '[data-grid]').each(function () {
            try {
                ...
                // 初始化模板
                let $tpl = $this.children('[data-grid-tpl]');
                if ($tpl.length > 0) {
                    $tpl.attr('id', elem + '-tpl');
                    $this.after($tpl);
                    // 获取参数
                    let options = parseAttrData($this.attr('lay-data'));
                    options.elem = '#' + elem;
