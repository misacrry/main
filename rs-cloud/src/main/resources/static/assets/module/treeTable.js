...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    let defaultOption = {
        ...
        tree: {
            ...
            getIcon: function (d) {                      // 自定义图标
                let haveChild = d[this.haveChildName];
    ...
    /** TreeTable类构造方法 */
    let TreeTable = function (options) {
        ...
        function each(data) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i][options.tree.childName]) {
                    let res = each(data[i][options.tree.childName]);
                    if (res) return res;
                }
            }
        }
    ...
    TreeTable.prototype.initOptions = function (opt) {
        ...
        // 恢复cols参数初始状态
        for (let m = 0; m < opt.cols.length; m++) {
            for (let n = 0; n < opt.cols[m].length; n++) {
        ...
        for (let i1 = 0; i1 < opt.cols.length; i1++) {
            let item1 = opt.cols[i1];
            for (let i2 = 0; i2 < item1.length; i2++) {
                ...
                if (item2.colGroup || item2.colspan > 1) {
                    ...
                    colIndex++;
                    let childIndex = 0;
                    for (let i22 = 0; i22 < opt.cols[i1 + 1].length; i22++) {
                        let item22 = $.extend({INIT_OK: true}, colDefaultOption, opt.cols[i1 + 1][i22]);
        ...
        if (typeof this.options.tree.getIcon === 'string') {
            let icon = this.options.tree.getIcon;
    ...
    TreeTable.prototype.init = function () {
        let options = this.options;
        let $elem = $(options.elem);  // 原始表格
        let tbFilter = options.elem.substring(1);  // 表格的filter
        // 第一次生成树表格dom
        ...
        if (options.toolbar === false || options.toolbar === undefined) {
            ...
            let tbRights = ['<div class="ew-tree-table-tool-right">'];
            for (let i = 0; i < options.defaultToolbar.length; i++) {
                let tbItem;
        ...
        // 表格尺寸设置
        let colgroupHtml = this.resize(true);
        // 生成thead
        let headHtml = '<thead>' + this.renderBodyTh() + '</thead>';
        ...
        if (options.height) {  // 固定表头
            ...
            if (options.height.indexOf('full-') === 0) {  // 差值高度
                let h = parseFloat(options.height.substring(5)) + components.$toolbar.outerHeight()
        ...
        function patchHide($tr) {
            let parentKey = $tr.data('parent'), pCol;
            if (!parentKey) return;
            let $parent = components.$table.children('thead').children('tr').children('[data-key="' + parentKey + '"]');
            let colspan = $parent.attr('colspan') - 1;
    ...
    TreeTable.prototype.bindEvents = function () {
        ...
        let options = this.options;
        let components = this.getComponents();
        let $allBody = components.$table.children('tbody');

        /* 行事件公共返回对象 */
        let member = function (ext) {
            // 获取行dom
            let $tr = $(this);
            if (!$tr.is('tr')) {
                let $temp = $tr.parent('tr');
                if ($temp.length > 0) $tr = $temp;
                else $tr = $tr.parentsUntil('tr').last().parent();
            }
            let data = that.getDataByTr($tr);  // 行对应数据
            let obj = {
                tr: $tr,
                data: data,
                del: function () { // 删除行
                    let index = $tr.data('index');
                    let indent = parseInt($tr.data('indent'));
                    // 删除子级
                    ...
                    // 更新后面同辈的index
                    let indexLength = (typeof index === 'number' ? 1 : index.split('-').length);
                    $tr.nextAll('tr').each(function () {
                        let $this = $(this);
                        if (parseInt($this.data('indent')) < indent) return false;
                        let _index = $this.data('index').toString().split('-');
                        _index[indexLength - 1] = parseInt(_index[indexLength - 1]) - 1;
                        $this.data('index', _index.join('-'));
                    });
                    // 删除当前行
                    let $pTr = $tr.prevAll('tr');
                    that.del(undefined, index);
                    $tr.remove();
                    that.renderNumberCol();  // 渲染序号列
                    // 联动父级多选框
                    $pTr.each(function () {
                        let tInd = parseInt($(this).data('indent'));
        ...
        $allBody.off('click.fold').on('click.fold', '.ew-tree-pack', function (e) {
            layui.stope(e);
            let $tr = $(this).parentsUntil('tr').last().parent();
            if ($tr.hasClass('ew-tree-table-loading')) return; // 已是加载中
            let haveChild = $tr.data('have-child');
            if (haveChild !== true && haveChild !== 'true') return; // 子节点
            let open = $tr.hasClass('ew-tree-table-open');
            let data = that.getDataByTr($tr);
        ...
        form.on('radio(' + components.radioFilter + ')', function (data) {
            let d = that.getDataByTr($(data.elem).parentsUntil('tr').last().parent());
        ...
        form.on('checkbox(' + components.checkboxFilter + ')', function (data) {
            ...
            let $cb = $(data.elem);
            let $layCb = $cb.next('.layui-form-checkbox');
            // 如果是半选状态，点击全选
            ...
            let $tr = $cb.parentsUntil('tr').last().parent();
            let d = that.getDataByTr($tr);
        ...
        $allBody.off('click.cell').on('click.cell', 'td', function (e) {
            ...
            let edit = $td.data('edit');
            let field = $td.data('field');
            if (edit) {  // 开启了单元格编辑
                layui.stope(e);
                if ($allBody.find('.ew-tree-table-edit').length > 0) return;
                let index = $td.data('index');
                let indent = $td.find('.ew-tree-table-indent').length;
                let d = that.getDataByTr($td.parent());
                if ('text' === edit || 'number' === edit) {  // 文本框
                    let $input = $('<input type="' + edit + '" class="layui-input ew-tree-table-edit"/>');
                    ...
                    $input.blur(function () {
                        ...
                        let rs = layui.event.call(this, MOD_NAME, 'edit(' + components.filter + ')', member.call(this,
                ...
                let rs = layui.event.call(this, MOD_NAME, 'cell(' + components.filter + ')', member.call(this,
                    {td: $td, field: field}));
        ...
        $allBody.off('dblclick.cellDouble').on('dblclick.cellDouble', 'td', function (e) {
            ...
            // 回调单元格双击事件
            let rs = layui.event.call(this, MOD_NAME, 'cellDouble(' + components.filter + ')', member.call(this,
                {td: $td, field: field}));
            if (rs === false) layui.stope(e);
        });
    ...
    TreeTable.prototype.getComponents = function () {
        let $view = $(this.options.elem).next('.ew-tree-table');  // 容器
        let filter = $view.attr('lay-filter');                    // 容器filter
        let $tHeadGroup = $view.children('.ew-tree-table-head');  // 表头部分容器
        let $tBodyGroup = $view.children('.ew-tree-table-box');   // 主体部分容器
        return {
            $view: $view,
            filter: filter,
    ...
    TreeTable.prototype.renderBodyData = function (data, d, $tr) {
        ...
        let htmlStr = this.renderBody(data, indent, d);
        if ($tr) {
            // 移除旧dom
            $tr.nextAll('tr').each(function () {
        ...
        if ($tr) {
            ...
            let $arrow = $tr.find('.ew-tree-pack').children('.ew-tree-table-arrow');
    ...
    TreeTable.prototype.renderBody = function (data, indent, parent, h) {
        ...
        let hide = parent ? !parent[options.tree.openName] : undefined;
    ...
    TreeTable.prototype.renderBodyTr = function (d, indent, hide, $tr) {
        ...
        let options = this.options;
        if (!indent) indent = 0;
        let haveChild = d[options.tree.haveChildName];
    ...
    TreeTable.prototype.renderBodyTd = function (d, indent, index, $td, col) {
        ...
        if (index === options.tree.iconIndex) {
            // 缩进
            for (let i = 0; i < indent; i++) icon += '<span class="ew-tree-table-indent"></span>';
    ...
    TreeTable.prototype.renderBodyTh = function () {
        ...
        $.each(options.cols, function (i1, item1) {
            ...
            $.each(item1, function (i2, item2) {
                ...
                // 标题
                let ca = '<input type="checkbox" lay-filter="' + components.chooseAllFilter + '" lay-skin="primary" class="ew-tree-table-checkbox"/>';
    ...
    TreeTable.prototype.resize = function (returnColgroup) {
        ...
        let options = this.options;
        let components = this.getComponents();
        let minWidth = 1, width = 1, needSetWidth = true, mwPercent = 0;
    ...
    TreeTable.prototype.checkSubCB = function ($tr, checked) {
        ...
        $trList.each(function () {
            if (parseInt($(this).data('indent')) <= indent) return false;
            let $cb = $(this).children('td').find('input[lay-filter="' + components.checkboxFilter + '"]');
            $cb.prop('checked', checked);
            $cb.removeClass('ew-form-indeterminate');
    ...
    TreeTable.prototype.checkParentCB = function ($tr) {
        let options = this.options;
        let components = this.getComponents();
        let d = this.getDataByTr($tr);
        let ckNum = 0, unCkNum = 0;
        ...
        let $cb = $tr.children('td').find('input[lay-filter="' + components.checkboxFilter + '"]');
    ...
    TreeTable.prototype.checkChooseAllCB = function () {
        ...
        let $cb = components.$view.find('input[lay-filter="' + components.chooseAllFilter + '"]');
        if (ckNum > 0 && unCkNum === 0) {  // 全选
            $cb.prop('checked', true);
            $cb.removeClass('ew-form-indeterminate');
            $cb.next('.layui-form-checkbox').addClass('layui-form-checked');
    ...
    TreeTable.prototype.getIndexById = function (id) {
        ...
        function each(data, pi) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i][options.tree.childName]) {
                    let res = each(data[i][options.tree.childName], pi !== undefined ? pi + '-' + i : i);
                    //值不为undefined才return
                    if (res) return res;
                }
            }
    ...
    TreeTable.prototype.expand = function (id, cascade) {
        let components = this.getComponents();
        let $tr = components.$table.children('tbody').children('tr[data-index="' + this.getIndexById(id) + '"]');
    ...
    TreeTable.prototype.setChecked = function (ids) {
        let that = this;
        let components = this.getComponents();
        let $radio = components.$table.find('input[lay-filter="' + components.radioFilter + '"]');
        if ($radio.length > 0) {  // 开启了单选框
            $radio.each(function () {
                let d = that.getDataByTr($(this).parentsUntil('tr').parent());
            ...
            components.$table.find('input[lay-filter="' + components.checkboxFilter + '"]').each(function () {
                ...
                let indeterminate = $cb.hasClass('ew-form-indeterminate');
                let d = that.getDataByTr($cb.parentsUntil('tr').parent());
    ...
    TreeTable.prototype.exportData = function (type) {
        ...
        if ('show' === type) {
            ...
            let content = encodeURIComponent(head.join(',') + '\r\n' + body.join('\r\n'));
            let contentType = ({csv: 'text/csv', xls: 'application/vnd.ms-excel'})[type];
    ...
    TreeTable.prototype.printTable = function () {
        let components = this.getComponents();
        let head = components.$tHead.children('thead').html();
        if (!head) head = components.$tBody.children('thead').html();
        let body = components.$tBody.children('tbody').html();
        let colgroup = components.$tBody.children('colgroup').html();
        let $html = $([
        ...
        function hideCol($temp) {
            let parentKey = $temp.data('parent'), pCol;
            if (!parentKey) return;
            let $parent = $html.children('thead').children('tr').children('[data-key="' + parentKey + '"]');
            let colspan = parseInt($parent.attr('colspan')) - 1;
            $parent.attr('colspan', colspan);
    ...
    TreeTable.prototype.toggleCol = function (show, field, key) {
        ...
        if (show === undefined) {
            components.$toolbar.find('.layui-table-tool-panel').remove();
            let cols = ['<ul class="layui-table-tool-panel">'];
            this.eachCols(function (i, item) {
            ...
            if (key) {
                let $td = components.$table.children('tbody').children('tr').children('[data-key="' + key + '"]');
                let $th = components.$table.children('thead').children('tr').children('[data-key="' + key + '"]');
                if (show) {
                ...
                // 同步更新数据
                let ks = key.split('-');
                let col = this.options.cols[ks[0]][ks[1]];
                col.hide = !show;
                ...
                function changeParent($temp) {
                    ...
                    if (!parentKey) return;
                    let $parent = components.$table.children('thead').children('tr').children('[data-key="' + parentKey + '"]');
                    let colspan = $parent.attr('colspan');
    ...
    TreeTable.prototype.filterData = function (ids) {
        ...
        let $trList = components.$table.children('tbody').children('tr');
        let indexList = [];
        ...
        for (let j = 0; j < indexList.length; j++) {
            let $tr = $trList.filter('[data-index="' + indexList[j] + '"]');
        ...
        /*$trList.not('.ew-tree-table-filter-hide').not('.ew-tree-tb-hide').each(function () {
            let index = $(this).data('index'), hide = true;
    ...
    TreeTable.prototype.del = function (id, index) {
        ...
        let indexList = (typeof index === 'number' ? [index] : index.split('-'));
    ...
    function toggleRow($tr) {
        ...
        if (open) {  // 折叠
            ...
            let hideInd;
            $tr.nextAll('tr').each(function () {
                let ind = parseInt($(this).data('indent'));
    ...
    function updateFixedTbHead($view) {
        let $headBox = $view.children('.ew-tree-table-head');
        let $tbBox = $view.children('.ew-tree-table-box');
        let sWidth = $tbBox.width() - $tbBox.prop('clientWidth');
    ...
    $(window).resize(function () {
        $('.ew-tree-table').each(function () {
            ...
            let $tbBox = $(this).children('.ew-tree-table-box');
            let full = $tbBox.data('full');
    ...
    $(document).on('mouseenter', '.ew-tree-table-cell.single-line', function () {
        let $content = $(this).children('.ew-tree-table-cell-content');
    ...
    $(document).on('mousedown', '.ew-tb-resize', function (e) {
        ...
        let w = $this.parent().parent().parent().parent().children('colgroup').children('col[data-key="' + key + '"]').attr('width');
        if (!w || w.toString().indexOf('%') !== -1) w = $this.parent().outerWidth();
        $this.data('width', w);
        $('body').addClass('ew-tree-table-resizing');
    }).on('mousemove', function (e) {
        let $rs = $('.ew-tree-table .ew-tb-resize[move="true"]');
        if ($rs.length === 0) return;
        layui.stope(e);
        let x = $rs.data('x');
        let w = $rs.data('width');
        let nw = parseFloat(w) + e.clientX - parseFloat(x);
        if (nw <= 0) nw = 1;
        // 更新实例options中的宽度
        let ins = _instances[$rs.parentsUntil('.ew-tree-table').last().parent().attr('lay-filter')];
        let key = $rs.parent().data('key');
        let ks = key.split('-');
        ...
        pidToChildren: function (data, idName, pidName, childName, pId) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (pidEquals(data[i][pidName], pId)) {
                    let children = this.pidToChildren(data, idName, pidName, childName, data[i][idName]);
